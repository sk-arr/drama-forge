...
**今日热点(首页)**
- 聚合 9 个热榜源:抖音、快手、微博、短剧榜、小红书、B站(默认开启),百度、头条、知乎(可选开启)
...
└── data/              # 运行时生成(配置/key/缓存/历史),已 gitignore,不进仓库
```
...
本项目使用 Claude Code 与 Codex 辅助开发:先完成行业调研与设计文档,再按「设计文档 → 分阶段开工提示词 → 逐任务实现/自检/commit → 实跑验收」的循环分六个阶段建造,全程 91 个自动化测试保障。完整的开发方法论与各阶段记录见 [docs/build-log.md](docs/build-log.md)。

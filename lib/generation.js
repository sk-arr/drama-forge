...
function parseIdeaCards(text) {
  return String(text || "").split(/\n(?=\d+\.\s*题目[:：])/).map((block) => {
    const title = (block.match(/题目[:：]\s*(.+)/) || [])[1] || "";
    const logic = (block.match(/一句话逻辑[:：]\s*(.+)/) || [])[1] || "";
    const genre = (block.match(/适配题材[:：]\s*(.+)/) || [])[1] || "其他自定义";
    if (!title.trim()) {
      return null;
    }
    return {
...
function ideasHistoryTitle(sourceName) {
  return `选题灵感 · ${sourceName || "今日热点"}`;
}
